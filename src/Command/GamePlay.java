...
public class GamePlay {
    ...
    public boolean useCard(String cardName){
        ...
        if (Cards.createCardByName(cardName).getType().equals(Type.Minion) && minions()==7) return false;
        if (isFirstPlayerTurn){
            ...
            if (secondPlayer!=null){
                for (Cards cards :secondPlayerHand) {
                if (cards.getName().equals(cardName))
                    card=cards;
            }
            secondPlayerHand.remove(card);
            secondPlayerGround.add(card);}
        }
    ...
    //Getter
    public Player getPlayerInRole() { return playerInRole; }
    public ArrayList<Cards> getFirstPlayerHand() { return firstPlayerHand; }
    public ArrayList<Cards> getFirstPlayerDeck() { return firstPlayerDeck; }
    public ArrayList<Cards> getFirstPlayerGround() { return firstPlayerGround; }
    public ArrayList<Cards> getSecondPlayerHand() { return secondPlayerHand; }
    public ArrayList<Cards> getSecondPlayerDeck() { return secondPlayerDeck; }
    public ArrayList<Cards> getSecondPlayerGround() { return secondPlayerGround; }
}
